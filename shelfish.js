(function() {
  const K='8942f1dc81e199d343c97639c0bbca67',R=()=>{
    document.querySelectorAll('ul:not([data-sh])').forEach(u=>{
      ...
      if(!f||!f.innerText.toLowerCase().includes('shelfish'))return;
      u.dataset.sh='1';const g=document.createElement('div');g.className='sh-g';
      Array.from(u.querySelectorAll('li')).slice(1).forEach(l=>{
        const [h,a,k]=l.innerText.split('|').map(s=>s.trim()),t=h.match(/\[(.*?)\]/)?.[1]?.toLowerCase()||'book',c=document.createElement('div');c.className='sh-c';
        c.innerHTML=`<div class="sh-t"><img class="sh-i"></div><b>${h.replace(/\[.*?\]/,'')}</b><p>${a}</p>${k?`<a href="${k}" class="sh-b">Review →</a>`:''}`;
